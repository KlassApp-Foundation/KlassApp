    	...
    	<div class="my-2" v-if="Object.keys(this.posts).length > 0">
			<div class="bg-white p-5 py-4" v-for="post in posts">
				<div class="">
		    		...
		    		<div class="bg-white ml-5 flex items-center justify-center cursor-pointer">
		    		...
		    		<div @click="showsidebar(id)" class="bg-white rounded-full ml-5 flex items-center justify-center cursor-pointer" v-if="post.created_by == auth_id">
		            ...
		            <div :id="id" class="hidden absolute right-0 bg-white shadow top-0 rounded my-5">
		                <div class="flex flex-col text-xs w-20 my-1" v-if="post.created_by == auth_id">
                ...
                <comment-list :url="url" :mode="mode" :id="post.id" type="classwall" :auth_id="auth_id"></comment-list>
	  		</div>
  		</div>
  		<div class="my-2 items-center" v-else>
...
<script>
	...
	export default{
		...
		methods:
		{
			getData()
			{
				axios.get(this.url+'/'+this.mode+'/standardLink/show/classwall/'+this.id).then(response => {
