<template>
  <div class="px-3 overflow-x-scroll lg:overflow-x-auto md:overflow-x-auto py-3" v-bind:class="[this.profile_tab==9?'block' :'hidden']">
...
<script>
  ...
  export default {
    ...
    methods:
    {
      getData()
      {
        axios.get('/'+this.mode+'/student/show/attendance/'+this.name).then(response => {
