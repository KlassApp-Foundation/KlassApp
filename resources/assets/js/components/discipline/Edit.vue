...
<script> 
    ...
    export default {
        ...
        methods:
        {
            getData()
            {
                axios.get('/admin/discipline/editlist/'+this.id).then(response => {
            ...
            setData()
            {
                if(Object.keys(this.list).length>0)
                {
                    ...
                    this.student_id       = this.list.student_id;
                    this.incident_date    = this.list.incident_date;
                    this.teacher_id       = this.list.teacher_id;
                    this.incident_detail  = this.list.incident_detail;
                    this.response         = this.list.response;
                    this.action_taken     = this.list.action_taken;
                    this.notify_parents   = this.list.notify_parents;
                    this.is_seen          = this.list.is_seen;
            ...
            submitForm()
            {
                ...
                axios.post('/admin/discipline/edit/'+this.id,formData,{headers: {'Content-Type': 'multipart/form-data'}}).then(response => {   
