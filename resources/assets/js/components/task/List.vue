<template>
    <div class="relative">
        ...
        <div class="flex flex-wrap lg:flex-row justify-between items-center">
            <div class="">
                <h1 class="admin-h1 my-3">Task</h1>
            </div>
            <div class="relative flex items-center w-9/12 lg:w-1/4 md:w-1/2 justify-end">
                <div class="flex items-center">
                    <div class="mb-1 w-8 mx-1">
                        <input type="checkbox" name="showCompleted" id="showCompleted" v-model="showCompleted" class="tw-form-control w-full" @change="showCompletedTask($event)">
                    </div>
            ...
            <div class="flex flex-wrap custom-table my-3 overflow-auto">
                <table class="w-full">
                    <thead class="bg-grey-light">
                        <tr class="border-b">
                            <th class="text-left text-sm px-2 py-2 text-grey-darker">Task Name</th>
                            <th class="text-left text-sm px-2 py-2 text-grey-darker">Task Date</th>
                            <th class="text-left text-sm px-2 py-2 text-grey-darker" width="5%">Actions</th>
                        </tr>
                    </thead>   
                    <tbody v-if="Object.keys(this.task).length > 0">
                        <tr class="border-b" v-for="task in task" >
                            ...
                            <td class="py-3 px-2">
                                <div class="flex items-center">                
                                    <a @click="taskEdit(task.id)" class="cursor-pointer">
        ...
        <div v-if="this.editshow == 1" class="modal modal-mask">
            <div class="modal-wrapper px-4">
                <div class="modal-container w-full  max-w-md px-8 mx-auto">
                    <div class="modal-header flex justify-between items-center">
                        <h2>Edit To Do</h2>
                        <button id="close-button" class="modal-default-button text-xl" @click="close()">&times;</button>
                    </div>
                    <div class="modal-body">
                        <div class="flex flex-col">
                            <div class="w-full lg:w-1/4"> 
...
<script>
    ...
    export default {
        ...
        methods:
        {
            getlist()
            {
                axios.get('/'+this.mode+'/task/list').then(response => {
            ...
            showCompletedTask(e)
            {
                if (e.target.checked) 
                {
                    ...
                    this.final = this.url+'/'+this.mode+'/task/list/?'+this.searchquery;
            ...
            {
                this.editshow=1;
                axios.get('/'+this.mode+'/task/'+id+'/edit').then(response => {
            ...
            updatetask(id)
            {
                axios.post('/'+this.mode+'/task/'+id+'/update',{
                    to_do_list:this.to_do_list,
                    task_date:this.task_date,
                }).then(response => {
                    this.success = response.data.message;
