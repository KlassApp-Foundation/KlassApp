    ...
    <div class="flex flex-wrap lg:flex-row justify-between items-center">
      ...
      <div class="relative flex items-center w-8/12 lg:w-1/4 md:w-1/4 justify-end">
        <div class="flex items-center">
          ...
          <a  class="no-underline text-white px-4 my-3 mx-1 flex items-center custom-green py-1 justify-center cursor-pointer">
      ...
      <div class="flex flex-wrap custom-table my-3 overflow-auto">
        <table class="w-full">
          <thead class="bg-grey-light">
            <tr class="border-b">
              <th class="text-left text-sm px-2 py-2 text-grey-darker">Visiting Purpose</th>
              <th class="text-left text-sm px-2 py-2 text-grey-darker">Visior Detail</th>
              <th class="text-left text-sm px-2 py-2 text-grey-darker">Visior Count</th>
              <th class="text-left text-sm px-2 py-2 text-grey-darker">Date Of Visit</th>
              <th class="text-left text-sm px-2 py-2 text-grey-darker">Entry Time</th>
              <th class="text-left text-sm px-2 py-2 text-grey-darker">Exit Time</th>
      ...
      <div class="modal-wrapper px-4">
        <div class="modal-container w-full  max-w-md px-8 mx-auto">
          ...
          <div class="modal-body">
            <div class="flex flex-col">
              <div class="w-full lg:w-1/4"> 
                <label for="to_do_list" class="tw-form-label">Decription</label>
              </div>
              <div class="my-2 w-full">
                <textarea type="text"  name="to_do_list" v-model="to_do_list" class="tw-form-control w-full"></textarea><br>
                <span v-if="errors.to_do_list"><p class="text-red-500 text-xs font-semibold">{{errors.to_do_list[0]}}</p></span>
            ...
            <div class="flex items-center">
              <div class="my-2 w-full lg:w-3/4">
                <input type="button" name="" value="Update" @click="updatetask(edittask.id)" class="btn btn-submit blue-bg text-white rounded px-3 py-1 cursor-pointer text-sm font-medium">
              </div>
            </div>
          </div>
        </div>
  ...
  methods:
  {
    ...
    showCompletedTask(e)
    {
      if (e.target.checked) 
      {
        ...
        this.final = this.url+'/teacher/task/list/?'+this.searchquery;
    ...
    {
      this.editshow=1;
      axios.get('/teacher/task/'+id+'/edit').then(response => {

        this.edittask=response.data;
        this.to_do_list=this.edittask.to_do_list;
        this.task_date=this.edittask.task_date;
    ...
    updatetask(id)
    {
      axios.post('/teacher/task/'+id+'/update',{
        to_do_list:this.to_do_list,
        task_date:this.task_date,
