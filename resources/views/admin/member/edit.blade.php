  ...
  @section('content')
    <div class="w-full lg:mx-2">
      <h1 class="admin-h1 my-3 flex items-center">
        <a href="{{ auth()->user()->usergroup_id === 5 ? route('teacher.classes.index') : url('/admin/student/show/'.$user->name) }}" title="Back" class="rounded-full bg-gray-100 p-2">
      ...
      <form method="POST" action="{{ auth()->user()->usergroup_id === 5 ? route('teacher.student.update', ['name' => $user->name]) : url('/admin/student/edit/'.$user->name) }}" enctype="multipart/form-data">
        @csrf
          <edit-member
            url="{{ url('/') }}"
            student_name="{{ $user->name }}"
            data_url="{{ auth()->user()->usergroup_id === 5 ? route('teacher.student.data', ['name' => $user->name]) : url('/admin/student/editStudent/'.$user->name) }}"
            validation_url="{{ auth()->user()->usergroup_id === 5 ? route('teacher.student.validation', ['name' => $user->name]) : url('/admin/student/edit/validationUser/'.$user->name) }}"
          ></edit-member>
          @if(false)
              ...
              <div class="tw-form-group w-full lg:w-1/2 md:w-1/2">
                <div class="lg:mr-8 md:mr-8">
                  ...
                  <div class="mb-2 w-full relative">
                    <input type="text" name="address" class="tw-form-control w-full" id="address" value="{{ $userprofile->address }}" required>
...
function initialize() 
{
    if (typeof google === 'undefined' || !google.maps || !document.getElementById('address')) {
        return;
    }
