    ...
    <div class="w-full">
      <div>
        <h1 class="admin-h1 my-3 flex items-center">
          <a href="{{ url('/admin/teacher/show/'.$user->name) }}" title="Back" class="rounded-full bg-gray-100 p-2">
          ...
          <span class="mx-3">Edit Teaching Staff</span>
        </h1>
      </div>
      @include('partials.message')
      <form method="POST" action="{{url('/admin/teacher/edit/'.$user->name)}}" enctype="multipart/form-data">
        @csrf

          <add-tab-teacher url="{{ url('/') }}" staff="teaching" teacher_name="{{ $user->name }}" type="edit"></add-tab-teacher>
          <portal-target name="edit_teacherprofile"></portal-target>
          ...
          <portal to="edit_address">
            <div class="flex flex-col lg:flex-row md:flex-row">
              ...
              <div class="tw-form-group w-full lg:w-1/2 md:w-1/2">
                <div class="lg:mr-8 md:mr-8">
                  <div id="map_canvas" class="tw-form-control w-full " style="height: 250px;">
                  </div> 
                </div>
              </div> 
