...
<div class="relative">
      ...
      <table class="ds-table-ledger">
         ...
         @if(count($booklending) != 0)
            <tbody class="bg-grey-light">
               ...
               @foreach($booklending as $lending)
                
               @php
                $class='';
                if($lending->return_date<$currentDate)
                {
                 $class='text-danger';
                }
              @endphp
               ...
               <tr class="border-t-2 border-b-2">  
                     ...
                     <td class="{{$class}}">{{ $lending->user[0]['FullName'] }}</td>
                     <td class="{{$class}}">{{ $lending->book_code_no}}</td>
                     <td class="{{$class}}">{{ $lending->library_card_no}}</td>
                     <td class="{{$class}}">{{ $lending->issue_date }}</td>
                     <td class="{{$class}}">{{ $lending->return_date }}</td>
                 
                      <td class="py-3 px-2">
                      <div class="flex items-center">
                        <a href="{{url('/library/booklending/edit/'.$lending->id)}}" class="capitalize rounded  font-medium">
  ...
  C443.754,73.631,440.467,81.575,434.603,87.419z"></path></g></g></svg>
                        ...
                        <a href="#" rel="{{url('/library/booklending/delete/'.$lending->id)}}" class="capitalize rounded  font-medium delete">
  ...
  C354.924,14.992,339.932,0,321.504,0z"></path></g></g></svg>
         ...
         @else
            <tbody class="bg-grey-light">
               <tr class="border-t-2 border-b-2">    
                  <td colspan="6" class="py-3 px-2"><p class="font-semibold text-s" style="text-align: center">No Records Found</p></td>
               </tr>
            </tbody>
         @endif
      </table>      
...
<script type="text/javascript">
   ...
   $(document).ready(function(){
      $('.delete').on('click', function(){
         ...
         swal({
            icon: "info",
            text: "Do you want to delete this BookLending Entry ?",
            buttons: {
               cancel: true,
               confirm: true,
            },
            ...
            if (willChange) 
            {
               $.ajax({
                  ...
                  success:function(data)
                  {
                     swal({
                        icon: "success",
                        text: "Book Lending Entry Deleted Successfully",
                     }).then(function(){
