...
28. **Open-PR scan at the start of every session, before beginning any task.** Immediately after the rule #19 sync and the rule #20 `knowledge.md` read, run a full open-PR scan — `gh api repos/KlassApp-Foundation/KlassApp/pulls?state=open` (all pages; don't rely on a cached list, a previous session's answer, or memory) — and report a real, current summary of every open PR in your first substantive reply before proceeding with the requested work: number, title, author, head branch, and mergeable status. The scan must cover **all** open PRs, not just ones relevant to the immediate task — the whole value is surfacing PRs nobody was thinking to look for. Flag any PR authored by anyone other than the project's own account with particular attention, given rule #25's human-review requirement for external contributions. Precedent (2026-09-26): [#682](https://github.com/KlassApp-Foundation/KlassApp/pull/682) and [#788](https://github.com/KlassApp-Foundation/KlassApp/pull/788) sat open, unreviewed, and unknown for an extended period while other work proceeded, and in an earlier session #819 was reported as "shipped" in a summary without the merge ever being confirmed — the gap rules #21 and this rule exist to close.
