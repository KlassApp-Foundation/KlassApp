...
14. **Deploy from the correct dedicated main worktree**, not an arbitrary local checkout — this project uses a worktree-based workflow (see `knowledge.md`'s note on the canonical `knowledge.md` path and worktree sync). Confirm you're on the right one before running the deploy script.
