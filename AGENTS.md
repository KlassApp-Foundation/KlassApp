...
28. **For any design, UI/UX, dashboard, or “how do others do this” research task, make **madewithlaravel.com** the standing first discovery step.** Enumerate the relevant category listings (API, Administrator, App, Boilerplate, Eloquent, Essentials, Plugins, Template, Testing, Tutorials, UI Components, Utility, Websites), filter to KlassApp's real stack (Laravel + Blade + Tailwind + Livewire + Alpine), and prefer projects on that stack over Filament/Bootstrap/Vue-React-only or paid “Pro” products. **Verify the actual `LICENSE` file** (`gh api repos/<owner>/<repo>` and `.../license`) before calling anything adoptable vs reference-only — a `composer.json` claim alone is not enough; GPL/AGPL/no-license = reference only. Log findings in the living research doc `docs/research/klassapp-external-comparison.md` (append/update — never fork a second disconnected doc), with the agent-authored independent verification record kept at `docs/ui-ecosystem-comparison.md` where the owner has directed separate authorship.
