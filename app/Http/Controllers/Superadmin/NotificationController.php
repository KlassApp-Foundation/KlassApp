...
/**
 * SiteAdmin (platform) notifications.
 *
 * Mirrors the other role controllers (Parent/Librarian/Alumni/Receptionist/
 * Accountant/Student/Teacher) and is entirely scoped to the authenticated user's
 * OWN notifications (`notifiable_id` = the caller) — never a caller-supplied id
 * and never a school-scoped query, so it cannot leak another tenant's rows.
 */
            ...
            if (Auth::user()) {
                ...
                foreach ($notifications as $notification) {
                    ...
                    if ((count($notification->data) > 0) && (isset($notification->data['data']))) {
                        // Payloads may be a plain string or ['data' => ..., 'type' => ...].
