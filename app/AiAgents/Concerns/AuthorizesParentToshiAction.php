...
trait AuthorizesParentToshiAction
{
    protected function authorizeParentAction(User $user): void
    {
        $response = Gate::inspect('toshi-parent-action', $user);
    ...
    protected function authorizeParentOrMessage(?User $user): ?string
    {
        if (! $user) {
        ...
        try {
            $this->authorizeParentAction($user);
