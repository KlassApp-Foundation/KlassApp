<?php

namespace App\AiAgents\Tools\Teacher;

use App\AiAgents\Concerns\AuthorizesTeacherToshiAction;
use App\Services\Toshi\TeacherActionService;
use Illuminate\Contracts\JsonSchema\JsonSchema;
use Laravel\Ai\Contracts\Tool;
use Laravel\Ai\Tools\Request;
use Stringable;

class ShowMyLeaveTool implements Tool
{
    use AuthorizesTeacherToshiAction;

    public function description(): Stringable|string
    {
        return 'Show one of the authenticated teacher\'s leave applications. Requires teacher-leave ownership.';
    }

    public function schema(JsonSchema $schema): array
    {
        return [
            'leave_id' => $schema->integer()->required(),
        ];
    }

    public function handle(Request $request): Stringable|string
    {
        $user = auth()->user() ?? request()->user();
        $error = $this->authorizeTeacherOrMessage($user);
        if ($error) {
            return $error;
        }

        $result = TeacherActionService::showMyLeave($user, [
            'leave_id' => $request->get('leave_id'),
        ]);

        return ($result['success'] ? '' : '❌ ').$result['message'];
    }
}
