...
- **Part B** (Vue 3.5.40 DEV build): see overview 1b.4 bullet. Notable: `/admin/students/blockedstudents` still **500** (S3 region missing — pre-existing, not Vue); used `/admin/teachers` person-card portal + `/admin/student/show/{name}` instead. Browser MCP could not open tabs (auth ok, navigate failed) — used existing Playwright Chromium via `PLAYWRIGHT_BROWSERS_PATH`.
