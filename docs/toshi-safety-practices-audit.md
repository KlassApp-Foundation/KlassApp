...
| **VPS-actual — model did NOT match** | On 2026-08-02, `toshi:llm-status` on prod reported model `meta/llama-3.1-8b-instruct` — **not** `deepseek-chat`. **Root cause (live, confirmed):** `.env` had `TOSHI_LLM_MODEL=meta/llama-3.1-8b-instruct` (an NVIDIA NIM model) while `OPENAI_COMPATIBLE_MODEL` was unset. `config/toshi.php` resolves `model → env('OPENAI_COMPATIBLE_MODEL', env('TOSHI_LLM_MODEL', 'deepseek-chat'))`, so prod sent an NVIDIA model name to the DeepSeek endpoint (`api.deepseek.com`). |
| **Live call result (broken → fixed)** | Before fix, a one-shot completion to prod config returned **HTTP 400** `{"error":{"message":"The supported API model names are deepseek-v4-pro or deepseek-v4-flash, but you passed meta/llama-3.1-8b-instruct.","type":"invalid_request_error"}}` — every role's agent would have failed at the provider. **Fix applied live:** appended `OPENAI_COMPATIBLE_MODEL=deepseek-v4-flash` to prod `.env` (backup `.env.bak.toshi-llm-*` kept), `optimize:clear`. Post-fix raw call → **HTTP 200** `TOSHI_LIVE_OK`, model echoed `deepseek-v4-flash`. |
...
| **Recommendation** | **(a)** Rerun `php artisan toshi:adversarial-live` against the real model (now `deepseek-v4-flash`) to revalidate the adversarial suite under production's actual model. **(b)** Reconcile the duplicate LLM env wiring — `TOSHI_LLM_*` (NVIDIA) vs `OPENAI_COMPATIBLE_*` (DeepSeek) currently co-exist and can silently change the effective model; pick a single source of truth. Adversarial schedule stays gated off in production until intentionally enabled. Production provider is now verified working.**
