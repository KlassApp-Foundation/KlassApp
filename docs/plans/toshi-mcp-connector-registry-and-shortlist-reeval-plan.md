...
| **3** | **Google Drive (Workspace MCP)** | **WAITING FOR GA (updated 2026-09-21 — see D.3)** | The **education jackslot**: schools live in Google Workspace for Education; KlassApp already ships Google OAuth (sign-in "Live" badge, config/services.php `google` block, existing `users.google_id`); Drive is in the marketing trio. The earlier "enroll + spike now" posture is **superseded**: full re-research (2026-09-21, Google's live docs) found Google's Developer Preview Program terms **contractually prohibit** giving customers outside our domain/organization access to anything built on the pre-GA Drive MCP server (Program Term iv — "for purposes of testing, I may not grant end users access, outside my domain or company…"), which blocks the per-school self-registration model this connector needs — a **legal** blocker, not an API-stability one. Even setting that aside, Drive doesn't yet clear the use-case bar Notion failed. Full status, watch-signals, and reopen criteria: **D.3**. |
...
| — | **Google Classroom (direct REST API)** | **CANDIDATE — read-only wave-1 (2026-09-21, see Thread A plan)** | New research pass (2026-09-21) found the **plain Classroom REST API is GA with standard OAuth 2.0 — no Developer Preview restriction** (the Term-iv blocker applies only to the MCP server). It clears the use-case bar the others failed: KlassApp *is* a school system and Classroom is the adjacent rostering/coursework system in the exact same domain. Recommended shape: self-hosted **local** MCP server wrapping the REST API (laravel/mcp 0.8.2 server support, pattern already production-proven by `SpikeSlackMockServer`) → plugs into the existing registry/audit/write-gate unchanged; 2 read tools (`google_classroom_list_courses`, `google_classroom_list_coursework`), sensitive scopes `classroom.courses.readonly` + `classroom.coursework.students.readonly`, **no roster/email/guardian scopes**, `write_tools=[]`, write gate armed. Calendar also GA-via-REST but deferred (read duplicates in-app `Events`); Drive-via-REST rejected (use case unmet + `drive.readonly` is a *restricted* scope → annual CASA assessment). Full plan: `docs/plans/toshi-direct-google-integrations-and-custom-connector-registry-plan.md` (Thread A). |
...
### D.3 Google Drive Developer Preview boundary (resolution #1) — **STATUS: WAITING FOR GA (2026-09-21)**

> **Supersedes the original "enroll + spike now" posture.** The research pass of 2026-09-21 (Google's live official docs — configure guide last updated 2026-09-18, Preview Program page, tools reference, file-eligibility guide, Google Cloud MCP docs) found the preview program contractually blocks KlassApp's rollout model, and the use case isn't cleared anyway. **Do not enroll, do not spike, do not build — wait for GA.** The original boundary spec below (items 1–5) remains the correct shape for whenever this is picked up post-GA.

**Why waiting (stated plainly):**

- **Contractual blocker (the decisive one).** Google's Developer Preview Program Terms, term (iv): *"for purposes of testing, I may not grant end users access, outside my domain or company, to developer applications that have been built using APIs prior to their GA announcement unless Google specifically states that I can request such permission and such permission has been granted."* Google's own FAQ: "Can I share my integration with my customers before the feature becomes generally available? A: No." KlassApp's per-school self-registration model (ANY school's admin independently connects their workspace) is therefore **prohibited under the preview terms** — this is a **legal** blocker, not an API-stability concern. Additionally, the Drive MCP service only works if Google has registered KlassApp's specific GCP project through the program's application process, and term (vi) grants Google use of any data submitted via Pre-GA APIs to improve those APIs — a data-handling exposure for student documents needing product/legal sign-off before even a pilot.
- **Use-case verdict (honest, same bar Notion failed):** even setting the contract issue aside, Drive doesn't yet clear that bar. The landing page's marketed write-flow ("Weekly report uploaded to /reports/term-2/") **duplicates functionality that already lives in-app** (report cards = in-app PDF service; bulletins = single PDF upload field; lesson plans = in-app `LessonPlan` + approval flow). The one credible use case is **read-side** — "find and summarize a doc from the shared staff Drive" (`search_files` + `read_file_content` cover it) — rated **weaker than Slack's grounded case** (which had a named product workflow), stronger than Notion's (zero marketing surface). Not cleared today.
- **Technical prerequisite (a cost whenever revisited, not something to fix now):** Google's Drive MCP server only speaks the **2026-07-28 stateless transport era** (per Google Cloud MCP docs: no `initialize` handshake, no `Mcp-Session-Id`). laravel/mcp 0.8.x speaks the older stateful era via the same dependency chain flagged in the earlier transport tripwire (R.8 / `McpTransportEraReverificationTest`, 2027-04-28). Slack serves the legacy era; **Drive would be the connector that forces the laravel/mcp 1.x upgrade** — budget that upgrade as a prerequisite of any Drive build.
- **Small drift items (harmless today, must be corrected before any real build):** the config catalog's `read_tools` names (`list_files`, `search_files`, `read_document`) don't match Google's actual tool names (`list_recent_files`, `search_files`, `read_file_content` — only `search_files` matches). Nothing calls them (`enabled=false`, no implementation), so no action now; fix in whatever future Drive PR happens.

**Update 2026-09-21 (Thread A research — see `docs/plans/toshi-direct-google-integrations-and-custom-connector-registry-plan.md`):** the *REST API* path (`drive.googleapis.com`) was checked as a way around the MCP-server restriction: contractually open (no preview terms on the plain API), but **rejected anyway** — the only credible use case (search/read the shared staff Drive) needs `drive.readonly`, a **restricted scope** requiring an annual third-party CASA security assessment, and the free scope (`drive.file`, non-sensitive) is per-file only and can't search. So the "waiting for GA" verdict is unchanged, and now doubly grounded: contractually blocked via MCP, use-case + verification-cost blocked via REST. The watch-signals below remain the decision triggers.

**Decision trigger — the real signals (replacing the arbitrary 2027-03-18 date for the *decision*):**

1. The **"Developer Preview" label disappears** from Google's Drive MCP docs (configure guide / tools reference), or the Workspace release notes announce Drive MCP GA.
2. A **GA announcement in the Google Workspace / Google Cloud MCP release notes**.
3. Any explicit statement that the **OAuth flow opens to apps without Preview Program registration** — the signal that specifically restores the Slack-style per-school self-registration shape.

The existing dated-tripwire TEST (`GoogleDrivePreviewReverificationTest`, 2027-03-18 + `TOSHI_GOOGLE_DRIVE_VERIFIED_AT`) **stays** — it remains a useful mechanical forcing function for periodic re-verification against Google's docs — but the **real decision trigger is the three signals above, not the date**. The 2026-09-21 research pass effectively performed that re-verification early.

**What would justify reopening (mirroring the Notion reopen criteria):** GA reached (any of the three signals fires) — **and then** the use-case bar must be re-cleared with a concrete, product-identified workflow (leading candidate: read-side staff-doc find/summarize in the shared Drive). **GA alone does not waive that bar.** When reopened: re-run the research pass (tool surface, scopes, terms — the tripwire's mechanism), budget the laravel/mcp 1.x upgrade, correct the catalog tool-name drift, then build through normal product review. Catalog entry and contract seam are retained unchanged.

**Original boundary spec (still the correct shape post-GA):**

D.1 rank-3's original "enroll + spike now" line resolved to this spec:
...
> **Update 2026-09-21 (Thread B scoping — see `docs/plans/toshi-direct-google-integrations-and-custom-connector-registry-plan.md` Thread B):** Tier 2 re-scoped in a research pass and **remains deferred** — but three gaps in the sketch above were found and recorded: (1) tool classification must run against a **KlassApp-curated manifest**, never live server-discovered tool lists (TOCTOU hole otherwise); (2) the preferred shape is a **reviewed community-template registry** (`trust_level='community_template'`) rather than raw school-supplied URLs — raw URLs stay deferred; (3) community templates must declare `requires_write_gates: true` as a hard build precondition (the master-switch-off = ungated failure mode is worse for untrusted servers than for first-party ones). Thread A (Google Classroom direct REST) was confirmed to need **none** of this — it ships as a Tier-1 static named client like Slack.
