...
class MarksToggleStatusCrossSchoolTest extends TestCase
{
    use RefreshDatabase;
    ...
    private \App\Models\Academics\Exam $examB;
    ...
    protected function setUp(): void
    {
        ...
        $this->schoolA = School::create([
            'name' => 'School A', 'slug' => 'mts-a',
            'email' => 'a@mts.test', 'phone' => '+256700000001',
            'status' => 1, 'registration_country' => 'Uganda',
        ]);
        $this->schoolB = School::create([
            'name' => 'School B', 'slug' => 'mts-b',
            'email' => 'b@mts.test', 'phone' => '+256700000002',
            'status' => 1, 'registration_country' => 'Uganda',
        ]);
        ...
        $this->teacherA = User::factory()->create([
            ...
            'email' => 'teacher.a@mts.test',
        ]);

        // Create exam in School B owned by teacherB
        ...
        $this->examB = \App\Models\Academics\Exam::create([
            'school_id' => $this->schoolB->id,
            'academic_year_id' => $yearB->id,
    ...
    /** @test */
    public function teacher_cannot_toggle_status_of_another_schools_exam()
    {
        $response = $this->actingAs($this->teacherA)
            ->withoutMiddleware(\App\Http\Middleware\MustBePrivilege::class)
            ->patch("/teacher/exam/{$this->examB->id}/change-status");
    ...
    /** @test */
    public function same_school_teacher_cannot_toggle_another_teachers_exam()
    {
        // Create a second teacher in School B who is NOT the exam owner.
        // Same school, different teacher — teacher_id check must reject.
        $otherTeacherB = User::factory()->create([
            'school_id' => $this->schoolB->id,
            'usergroup_id' => 5,
            'email' => 'other.teacher.b@mts.test',
        ]);

        $response = $this->actingAs($otherTeacherB)
            ->withoutMiddleware(\App\Http\Middleware\MustBePrivilege::class)
            ->patch("/teacher/exam/{$this->examB->id}/change-status");
