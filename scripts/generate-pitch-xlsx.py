...
ws1.cell(row=10, column=2, value="klassapp.xyz  ·  hello@klassapp.xyz").font = Font(name='Arial', color=TEXT_GRAY, size=11)
...
ws2.cell(row=1, column=1).alignment = left_align
...
ws2.cell(row=1, column=5).alignment = center_align
...
for i, b in enumerate(bullets):
    ...
    ws2.cell(row=3+i, column=2, value=b).font = Font(name='Arial', color=TEXT_DARK, size=12)
    ws2.cell(row=3+i, column=2).alignment = left_align
    ws2.row_dimensions[3+i].height = 22

stats = [("98%", "WhatsApp penetration\namong smartphone users", GREEN), ("$40", "Smartphones now\ncommon in Uganda", BLUE), ("70%+", "School fees paid\nvia mobile money", AMBER)]
for i, (num, label, color) in enumerate(stats):
    ...
    ws2.cell(row=r+1, column=5, value=label).font = Font(name='Arial', color=TEXT_DARK, size=9)
...
ws3.cell(row=1, column=1).alignment = left_align
...
ws3.cell(row=1, column=5).alignment = center_align
...
features = [
    "Parents get grades, fees, child's health updates, attendance, and more via WhatsApp. No app. No login. Just a message.",
    "Schools get dashboards, automated campaigns, and delivery tracking.",
    "Works on top of existing school systems — no rip-and-replace. Just plug and connect.",
    "Two-way: parents check balances, report absences, and request results.",
    "Integrates with SchoolPay, existing ERPs, and Uganda's national LIN database.",
...
for i, f in enumerate(features):
    ...
    ws3.cell(row=4+i, column=2).alignment = left_align
    ws3.row_dimensions[4+i].height = 22

metrics = [("0-15 UGX", "per message\n(vs 30 UGX SMS)", GREEN), ("95%+", "delivery rate\n(vs 70% SMS)", BLUE), ("97%", "gross margin", AMBER)]
...
ws4.cell(row=7, column=2, value="Delivery Dashboard  ·  Analytics  ·  Failure Escalation  ·  AI Import  ·  SchoolPay Integration").font = Font(name='Arial', color=TEXT_DARK, size=11)
...
feat = [
    "Schools publish grades, fees, or attendance → KlassApp formats and delivers instantly via WhatsApp.",
    "Parents reply with keywords (\"grades\", \"fees\", \"absent\") → instant AI-powered response.",
    ...
    "Integrates with Uganda's national LIN system for instant student-to-parent linking.",
    "SchoolPay integration: automated fee status, payment confirmations, balance checks.",
]
for i, f in enumerate(feat):
    ws4.merge_cells(f'B{9+i}:F{9+i}')
    ws4.cell(row=9+i, column=2, value=f).font = Font(name='Arial', color=TEXT_DARK, size=11)
    ws4.cell(row=9+i, column=2).alignment = left_align
...
cols_data = [
    ("WhatsApp-First", GREEN, ["Works inside the app parents\nalready use daily", "Zero friction: no download, no\naccount, no password", "98% penetration in target\nmarket", "Zero-rated on MTN & Airtel —\nfree to receive"]),
    ("LIN Integration", BLUE, ["Connects to Uganda's national\nstudent ID database", "Parents self-register by texting\ntheir child's LIN", "Creates a nationwide parent\nnetwork — not just per-school", "Moat: no competitor connects to\nthe national LIN system"]),
    ("Platform vs. Point Solution", AMBER, ["Layers on top of existing systems\n— no rip-and-replace", "Integrations: SchoolPay, ERPs,\ngradebooks, LIN", "Roles: admin, teacher, bursar,\nlibrarian, nurse, secretary", "Student data follows the child\nacross school transfers"]),
]
for ci, (col_title, col_color, items) in enumerate(cols_data):
    ...
    for ii, item in enumerate(items):
        ...
        ws5.cell(row=r, column=scol).alignment = left_align
        ws5.cell(row=r, column=scol).fill = light_fill
...
stats = [("5", "schools live\nin testnet", GREEN), ("150+", "teachers active\non the platform", BLUE), ("2,500+", "students on\nthe system", AMBER)]
for i, (num, label, color) in enumerate(stats):
    col = 2 + i
    fill = PatternFill(start_color=color, end_color=color, fill_type='solid')
    ...
    ws6.row_dimensions[4].height = 25
...
ws7.cell(row=1, column=1, value="MARKET — TAM / SAM / SOM").font = Font(name='Arial', bold=True, color=WHITE, size=16)
...
defs = [
    ("TAM — Total Addressable Market", "Everyone who could use the product. All 74,000 schools × 500 parents = 37M parent-school connections."),
    ("SAM — Serviceable Addressable Market", "The subset you can actually reach. ~30,000 schools with smartphone access and data = 15M connections."),
    ("SOM — Serviceable Obtainable Market", "What you can realistically capture. Target of ~5,000 early-adopter schools in 3 years = 2.5M connections."),
]
for i, (label, desc) in enumerate(defs):
    ws7.cell(row=5+i, column=2, value=label).font = Font(name='Arial', bold=True, color=BLUE, size=11)
    ws7.merge_cells(f'C{5+i}:D{5+i}')
    ws7.cell(row=5+i, column=3, value=desc).font = Font(name='Arial', color=TEXT_DARK, size=10)
    ws7.cell(row=5+i, column=3).alignment = left_align
    ws7.row_dimensions[5+i].height = 30
...
ws8.merge_cells('B11:C11')
ws8.cell(row=11, column=2, value="PRICING TIERS").font = Font(name='Arial', bold=True, color=DARK, size=13)
tiers = [
...
for i, (name, price, usd, desc, color) in enumerate(tiers):
    col = 2 + i
    fill = PatternFill(start_color=color, end_color=color, fill_type='solid')
    ws8.merge_cells(start_row=13, start_column=col, end_row=14, end_column=col)
    ws8.cell(row=13, column=col, value=name).font = Font(name='Arial', bold=True, color=WHITE, size=14)
    ws8.cell(row=13, column=col).fill = fill
    ws8.cell(row=13, column=col).alignment = center_align
    ws8.merge_cells(start_row=15, start_column=col, end_row=15, end_column=col)
    ws8.cell(row=15, column=col, value=f"{price}\n({usd})").font = Font(name='Arial', bold=True, color=TEXT_DARK, size=11)
    ws8.cell(row=15, column=col).alignment = center_align
    ws8.cell(row=15, column=col).fill = light_fill
    ws8.merge_cells(start_row=16, start_column=col, end_row=16, end_column=col)
    ws8.cell(row=16, column=col, value=desc).font = Font(name='Arial', color=TEXT_GRAY, size=9)
    ws8.cell(row=16, column=col).alignment = center_align
    ws8.cell(row=16, column=col).fill = light_fill
...
gtm = [
    ...
    ("Partnerships", "Ministry of Education, school associations, education NGOs, mobile money providers, School ERPs, and SchoolPay."),
...
ws9.cell(row=14, column=2, value="200 schools  →  $53K ARR  →  $35K net  →  Path to Series A").font = Font(name='Arial', bold=True, color=GREEN, size=12)
...
ws10.cell(row=5, column=2, value="Mucunguzi Moses — Founder\n\n2x Founder. 5 years engineering experience. Deep understanding of Uganda's education and payments ecosystem.").font = Font(name='Arial', color=TEXT_DARK, size=11)
...
ws10.cell(row=9, column=2, value="Mugisha Elijah — Co-founder & CTO\n\nFullstack engineer. Built KlassApp platform from the ground up. Leads technical architecture and product development.").font = Font(name='Arial', color=TEXT_DARK, size=11)
...
ws10.cell(row=5, column=5, value="Raising $30K (UGX 100M) — Pre-Seed").font = Font(name='Arial', bold=True, color=WHITE, size=13)
...
for i, (pct, desc) in enumerate(alloc):
    r = 7 + i
    ws10.cell(row=r, column=5, value=f"{pct}  {desc}").font = Font(name='Arial', color=TEXT_DARK, size=11)

ws10.merge_cells('B13:E13')
ws10.cell(row=13, column=2, value="Target: 200 schools in 18 months  →  $53K ARR  →  $35K net  →  Path to Series A").font = Font(name='Arial', bold=True, color=GREEN, size=13)
...
ws11.cell(row=4, column=2).alignment = center_align
...
ws11.cell(row=8, column=2).alignment = center_align
...
ws11.cell(row=10, column=2).alignment = center_align

ws11.merge_cells('B12:F12')
ws11.cell(row=12, column=2, value="klassapp.xyz  ·  hello@klassapp.xyz  ·  WhatsApp: +256 767 538805").font = Font(name='Arial', color=TEXT_GRAY, size=11)
ws11.cell(row=12, column=2).alignment = center_align
...
ws11.cell(row=15, column=2).alignment = center_align
